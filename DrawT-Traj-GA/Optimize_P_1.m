function [P_un,P_sn,g_x]= Optimize_P_1(P_u1,P_s1,tau_j,eta_j,N,q_j_1)
...
y_j = sqrt( (delta_t.^4 + D.^2.*(sum((q_j_12 - q_j_11).^2)).^2 ).^0.5- D*sum((q_j_12 - q_j_11).^2) );
...
%% Run the CVX to solve the problem
    cvx_begin %quiet
       ...
       for n1=1:N
            ...
            P_1.*sqrt( (delta_t.^4 + D.^2.*(sum((q_j_1(:,n1+1) - q_j_1(:,n1)).^2)).^2 ).^0.5- D*sum((q_j_1(:,n1+1) - q_j_1(:,n1)).^2) )+P_p.* pow_pos(norm(q_j_1(:,n1+1)-q_j_1(:,n1)),1.5)./(delta_t.^2);
       end
       %% Maximization %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
       minimize (-g_x)  
       %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   
       subject to   
        ...
        for n=1:N               
            ...
            P_c1n = P_c1(:,[1:n]);  
            P_unn = P_un(:,[1:n]);  
            sum(Sum_E_Fly+tau_j1.*delta_t.*(P_c1n+P_unn))<=sum((miu.*(1-tau_j1).*delta_t.*omega_0.*P_h))./z_1jn;
        end
        %%
    cvx_end
    iter = iter + 1;
