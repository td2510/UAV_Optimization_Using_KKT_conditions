function [q,z_1,z_2,y_n,g_x] = Trajectory_Linear3(q_j_21,q_j_22,z_1j,z_2j,tau_j,eta_j,N)
...
%    variable E_fly(1,N)
   ...
   Theta_1 = log2(1+Theta_0.*P_s./z_1j)-Theta_0.*P_s.*(z_1-z_1j)./z_1j./log(2)./(z_1j+Theta_0.*P_s);
...
%    Theta_3 =  log2(1+Theta_0.*P_u./z_2j)-Theta_0.*P_u.*(z_2-z_2j)./z_2j./log(2)./(z_2j+Theta_0.*P_u); % Data transmission rate from UAV to d if it cached a part of f file 
...
%     end
   ...
   subject to   
    ii = 1: N;
    norm(q(:,ii+1)-q(:,ii)) <= V_max.*delta_t; % constraint 21e
    q(:,1)==q_I2; q(:,N+1)==q_F2; % Constraint 21f        
    H^2 +  sum( (q(:,ii+1)-w_d1).^2 ) <= (z_2).^(2/alpha); % Constraint 47c
    H^2 +  sum( (q(:,ii+1)-w_s1).^2,1 ) <= (z_1).^(2/alpha); % Constraint 47b    
    sum(tau_j.*delta_t.*Theta_1) + sigma.*S >= g_x; % Constraint 53c
%     sum(tau_j.*delta_t.*Theta_1) >= S.*(1-sigma); % Constraint 53c
    g_x >= S; % Constraint 53d
    ...
    for n=1:N               
        ...
        sum(Sum_E_Fly+tau_j1.*delta_t.*(P_c+P_u))<=sum(miu.*(1-tau_j1).*delta_t.*omega_0.*P_h.*z1_Taylor_n);% Constraint 53f
    end
    %%
cvx_end
iter = iter + 1;
